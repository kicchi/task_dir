...
def MNIST():
	...
	return N_train, N_validation, batch_size, n_hiddens, epochs, X, Y
...
def IRIS():
	...
	Y = np.eye(3)[y.astype(int)] #1-of-K表現に変換
	
	return N_train, N_validation, batch_size, n_hiddens, epochs, X, Y

	
if __name__ == '__main__':
	#N_train, N_validation, batch_size, X, Y = MNIST()
	N_train, N_validation, batch_size, n_hiddens, epochs, X, Y = IRIS() 
	...
	ax_acc.plot(range(epochs), history['val_acc'],label='acc',color='red')
	ax_loss = ax_acc.twinx()
	ax_loss.plot(range(epochs), history['val_loss'],label='loss',color='gray')
	plt.xlabel('epochs')
	#plt.show()
	plt.savefig('iris_tensorflow.eps')
